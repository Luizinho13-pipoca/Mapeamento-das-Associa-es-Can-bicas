...
SHEETS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSuIqquyZGuIhzfTHYYHwWdWB05T8UN_YrE_fE0R0LEvbFZcK83zBnAadwGdNOL0g/pub?output=csv"

# ✅ link do formulário
FORM_URL = "https://docs.google.com/forms/d/e/1FAIpQLSdZJ3ARpU9ej_xyanT2wfWyotBC_WMY_jsZhRgRRXmzuLylew/viewform"

# >>> Paleta roxa institucional para os mapas
PURPLE_SCALE = [
    [0.0,  "#EDE7F6"],
    [0.2,  "#D1C4E9"],
    [0.4,  "#B39DDB"],
...
# >>> Tema plotly (compatível com versões recentes)
...
# ---- normalização de município (para bater com GeoJSON)
...
def make_md_link(url, label: str) -> str:
    """Retorna link em Markdown ou string vazia. (DataTable precisa presentation='markdown')"""
    if pd.isna(url):
        return ""
    s = str(url).strip()
    if not s:
...
# -------------------------
# GeoJSON municipal por UF (cache)
# -------------------------
GEO_MUN_CACHE = {}
...
def guess_prop_key(geojson: dict):
    """Tenta descobrir o campo em properties que contém o nome do município."""
    try:
...
df_raw = load_sheet(SHEETS_CSV_URL)
df = df_raw.copy()
...
COL_FONTE = "Fonte da informação (FACT; Kaya Mind; Lista dos Advogados; Busca livre na internet)"
COL_PESQ = "Pesquisador responsável pelo preenchimento (Daniel, Luiz, Mariana, Mário, Natalia, Ramon, Rebeca)"
COL_OBS = "Observação"
...
# >>> Ajuste: altura do header usada pra calcular a área “do painel”
HEADER_H = "96px"   # se precisar, ajuste pra 88px / 104px
...
app.layout = html.Div([
    ...
    # ✅ ALTERADO: container principal tem altura de viewport (não cresce “infinito”)
    html.Div([

        # SIDEBAR (filtros + caixa no rodapé)
        html.Div([

            # filtros com scroll interno
            html.Div([
                html.H4("Filtros"),
            ...
            # caixa no rodapé (não fixa na tela; só fica no fim da coluna)
            html.Div([
            ...
            # (opcional) um espacinho no final da sidebar
            ...
            "height": "100%"   # ✅ herdado do container principal
        }),

        # CONTEÚDO PRINCIPAL (scroll aqui, quando precisar)
        html.Div([
            ...
            "flex": 1,
            "padding": "16px",
            "overflowY": "auto",   # ✅ scroll do conteúdo (não da página toda)
            "minHeight": 0
        }),

    ], style={
        "display": "flex",
        "height": f"calc(100vh - {HEADER_H})",  # ✅ trava altura total do painel
        "minHeight": 0
    })
...
def update_dashboard(f_uf, f_mun, f_serv, f_links):
    ...
    for c in (f_serv or []):
        if c not in d.columns:
            continue
        if "atendimento médico" in c.lower():
            d = d[d[c].isin(["sim", "MP"])]
        else:
            d = d[d[c].isin(["sim"])]
    ...
    # KPIs
    total = len(d)
    n_ufs = safe_col(d, COL_UF).nunique()
    n_muns = safe_col(d, COL_MUN).nunique()
    ...
    # Mapa (UF)
    # -------------------------
    UF_COL_MAPA = "uf_sigla"
    try:
        if UF_COL_MAPA in d.columns:
            por_uf = (
                d.groupby(UF_COL_MAPA)
                .size()
                .reset_index(name="n")
                .dropna(subset=[UF_COL_MAPA])
            )
            ...
            fig_mapa = blank_fig("Mapa indisponível: coluna uf_sigla não encontrada.")
    ...
    # -------------------------
    # Mapa municipal (por UF selecionada)
    # -------------------------
    try:
    ...
    try:
        if COL_MUN in d.columns:
            rmun = d[COL_MUN].astype(str).value_counts().head(20).reset_index()
    ...
    fig_fonte = blank_fig("Ranking de fonte: removido dos filtros (opcional manter/retirar).")
    ...
    return (
        kpis, fig_mapa, fig_mapa_mun, fig_mun, fig_fonte,
        data, columns,
